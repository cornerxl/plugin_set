...
(function () {
    ...
    plugin_04001.prototype.init = function (view) {
        var me = this;
        var template = `<div class="nd-plugin-collapse-box nd-plugin-slideimg-panel" x-model="data.collapse">
    ...
    plugin_04001.prototype.render = function (view) {
        var me = this;
        ...
        function delayRender() {
            ...
            if (data.data.collapse.isCollapse) {
            ...
            var clickEvent = function (e, d, v) {
                if (data.data.collapse.isCollapse) {
                    ...
                    data.data.collapse.isCollapse = false;
                } else {
                    data.data.collapse.isCollapse = true;
                    DD.css(collapseCon, 'height', collapseConInfoHeight);
                    DD.css(collapseHead, 'border-bottom', '1px solid #ddd');
                }
                DD.css(collapseCon, 'transition-property', 'height');
                DD.css(collapseCon, 'transition-duration', '500ms');
                DD.css(collapseCon, '-webkit-transition-property', 'height');
                DD.css(collapseCon, '-webkit-transition-duration', '500ms');
            }
            //点击事件
            new DD.Event({
                eventName: 'click',
